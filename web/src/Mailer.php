...
class Mailer {
     ...
     * @param string|array<string> $to
     */
    public function sendUploadLink(string|array $to, string $token, string $themeName, string $baseUrl): ?string {
        $link = rtrim($baseUrl, '/') . '/verify?token=' . urlencode($token);
        $html = <<<HTML
        <div style="font-family: system-ui, sans-serif; max-width: 480px; margin: 0 auto;">
            <h2 style="color: #333;">Publish your Clarity theme</h2>
            <p>Click the link below to confirm and publish your theme <strong>{$themeName}</strong>. This link expires in 15 minutes.</p>
            <p><a href="{$link}" style="display: inline-block; padding: 12px 24px; background: oklch(72% 0.16 230); color: #fff; text-decoration: none; border-radius: 6px;">Publish theme</a></p>
            <p style="color: #888; font-size: 13px;">If you didn't upload this theme, you can safely ignore this email.</p>
        </div>
        HTML;
        return $this->send($to, 'Publish your Clarity theme: ' . $themeName, $html);
    }
}
